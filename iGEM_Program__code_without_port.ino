...
void light_control::loop()
{
  analogWrite(pinLED_610nm,254);
  sleep(2000);
  NTU[0] = float(BH1750());        /*tell the NTU of culture*/
  sleep(500);
...
{
  if (NTU[0] <= 6.44 && NTU[1] <= 6.44 &&  NTU[2] <= 6.44) digitalWrite(pinLED_525nm,HIGH); /*If all value in NTU[] are lower than 6.44, Open 525nm LED*/
  
}

/*-----------------------heat_control--------------------------*/
float desiredTemp = 30.00 ; //desired Temperature
